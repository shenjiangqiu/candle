...
use candle_transformers::models::llama::{self as model};
...
fn main() -> Result<()> {
    ...
    for (i, prompt) in fetch_all_user_prompt()
        ...
        .enumerate()
    {
        let test_folder = format!("tests/llama-collect/",);
        println!("\n\n-----------------------\ngenerating for prompt: {}", i);
        let file_path = format!("{test_folder}/test_{}", i);
        fs::create_dir_all(&file_path).unwrap();

        let real_prompt = build_prompt(&prompt);
        ...
        // when finished, save the att record
        let att_record = cache.all_att.lock().unwrap();

        let att_record = att_record.deref();
        let file = Path::new(&file_path).join("att_history.bin");
        let file = File::create(file)?;
        let buffer = BufWriter::new(file);
        bincode::serialize_into(buffer, att_record)?;
    }
    Ok(())
}
