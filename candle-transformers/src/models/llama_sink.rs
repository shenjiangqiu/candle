...
pub const MAX_SINK_SIZE: usize = 4;
#[derive(Deserialize)]
...
#[derive(Clone)]
struct KVStore {
    sink_k: Vec<Option<Tensor>>,
    sink_v: Vec<Option<Tensor>>,
}

impl KVStore {
    pub fn new(blocks: usize) -> Self {
        Self {
            sink_k: vec![None; blocks],
            sink_v: vec![None; blocks],
        }
    }

    pub fn clear(&mut self) {
        self.sink_k.iter_mut().for_each(|x| *x = None);
        self.sink_v.iter_mut().for_each(|x| *x = None);
    }
    ...
    pub fn get_tensor_at(&self, block_idx: usize, index_pos: usize) -> (Tensor, Tensor) {
        ...
        let result_k = self.sink_k[block_idx].clone().unwrap();
        let result_v = self.sink_v[block_idx].clone().unwrap();
    ...
    fn save_sink(&mut self, block_idx: usize, k: &Tensor, v: &Tensor) -> Result<()> {
        let sink_k = &mut self.sink_k[block_idx];
        if let Some(sink_k) = sink_k {
            *sink_k = Tensor::cat(&[&*sink_k, k], 1)?.contiguous()?;
        } else {
            *sink_k = Some(k.clone());
        }
        let sink_v = &mut self.sink_v[block_idx];
        if let Some(sink_v) = sink_v {
            *sink_v = Tensor::cat(&[&*sink_v, v], 1)?.contiguous()?;
        } else {
            *sink_v = Some(v.clone());
        }
        Ok(())
...
impl CausalSelfAttention {
    ...
    fn forward(
        ...
        assert_eq!(b_sz, 1);
        let mut kvs = self.cache.kvs.lock().unwrap();

        // we need put the value into cache before transpose and rope
        if self.cache.use_kv_cache {
            kvs.save_tensor(k.clone(), v.clone(), index_pos, block_idx)
                .unwrap();
        }

        if self.cache.use_kv_cache {
            if index_pos < MAX_SINK_SIZE + MAX_WINDOW_SIZE {
                let first_part = (index_pos + seq_len).min(MAX_SINK_SIZE + MAX_WINDOW_SIZE);
                // perform attention on first part
                debug!("handleing first part: index_pos: {index_pos}, first_part:{first_part}");
                let (cache_k, cache_v) = kvs.get_tensor_at(block_idx, first_part);

                assert_eq!(cache_k.dims3().unwrap().1, first_part);
                let reshaped_k = cache_k
                    .reshape((b_sz, first_part, self.num_key_value_heads, self.head_dim))?
                    .transpose(1, 2)?;
                ...
                let reshaped_q = q
                    .narrow(1, 0, first_part - index_pos)
                    .unwrap()
                    .reshape((
                        b_sz,
                        first_part - index_pos,
                        self.num_key_value_heads,
                        self.head_dim,
                    ))?
                    .transpose(1, 2)?;

                let rotated_q = self.apply_rotary_emb(&reshaped_q, index_pos)?;
                debug!("computing first part:{} to {}", index_pos, first_part);
                let mut y = self.compute_attention(
                    rotated_k,
                    reshaped_v,
                    rotated_q,
                    first_part - index_pos,
                ...
                for index_pos_tailing in (first_part + 1)..=(index_pos + seq_len) {
                    // in this part, the data is not continues.
                    let single_y = self.handle_q_one_by_one(
                        &kvs,
                        block_idx,
                        index_pos_tailing,
                        index_pos_tailing - 1 - index_pos,
                        b_sz,
                        &q,
                        hidden_size,
                    )?;
                    y = Tensor::cat(&[&y, &single_y], 1)?;
                }

                let y = self.o_proj.forward(&y)?;
                Ok(y)
            } else {
                // the index_pos is larger than MAX_SINK_SIZE + MAX_WINDOW_SIZE
                let y = (index_pos..(index_pos + seq_len))
                    .enumerate()
                    .map(|(q_pos, index_pos)| {
                        let y = self
                            .handle_q_one_by_one(
                    ...
                    .reduce(|a, b| Tensor::cat(&[&a, &b], 1).unwrap())
                    .unwrap();
                let y = self.o_proj.forward(&y)?;
                Ok(y)
            }
        } else {
            // do not use the cache, simplly compute the attention
            let k = k
            ...
            let y = self.compute_attention(k, v, q, seq_len, b_sz, hidden_size)?;
            let y = self.o_proj.forward(&y)?;
            Ok(y)
        }
    }
