...
fn special_handle(k: &Tensor, part_length: usize) -> Tensor {
    ...
    let first_part = k.narrow(2, 0, MAX_SINK_SIZE).unwrap();
    let seq_len = k.dims4().unwrap().2;
    assert!(
        part_length <= seq_len,
        "part_length: {part_length}, seq_len: {seq_len}"
    );
    // println!("seq_len: {seq_len}");
    let second_part = k
        .narrow(2, part_length - MAX_WINDOW_SIZE, MAX_WINDOW_SIZE)
        .unwrap();
    let middle_part = k
        .narrow(
            2,
            MAX_SINK_SIZE,
            part_length - MAX_WINDOW_SIZE - MAX_SINK_SIZE,
        )
        .unwrap();
    let middle_part = msb_only(middle_part);
    let mut result = Tensor::cat(&[first_part, middle_part, second_part], 2).unwrap();
    if part_length < seq_len {
        let last_part = k.narrow(2, part_length, seq_len - part_length).unwrap();
        result = Tensor::cat(&[result, last_part], 2).unwrap();
    }
    assert_eq!(result.dims4().unwrap().2, seq_len);
    result.contiguous().unwrap()
}
