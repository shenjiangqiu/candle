...
impl StreamCache {
    ...
    pub fn clear_kv_cache(&self) {
        ...
        // let mut masks = self.masks.lock().unwrap();
        // masks.clear();
        let mut global_mask = self.global_mask.lock().unwrap();
        global_mask.iter_mut().for_each(|x| {
            x.iter_mut()
                .for_each(|y| y.iter_mut().for_each(|z| *z = true))
        });
    ...
    fn update_global_mask(
        ...
        for header in 0..32 {
            ...
            if score < evict_threshold {
                cache[header][tokenindex_to_be_evicted] = false;
...
/// return full_tensor mask and MSB mask
fn special_handle(
    part_length: usize,
    block_mask: &Vec<[bool; MAX_SEQ_LEN]>,
    header_dim: usize,
) -> (Tensor, Tensor) {
    use rayon::prelude::*;
    let origin_k_mask = block_mask
        .par_iter()
        .map(|x| {
            x.par_iter()
                .take(part_length)
    ...
    let msb_k_mask = block_mask
        ...
        .collect::<Vec<_>>();
    let origin_k_mask = vec![origin_k_mask];
    let msb_k_mask = vec![msb_k_mask];
